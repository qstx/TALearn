Shader "TALearn/TessellationAndGeometry/GrassGroundShader"
{
    Properties
    {
        ...
        _TessellationUniform("Tessellation Uniform",Range(1,20))=10
        //风
    ...
    SubShader
    {
        ...
        geometryOutput VertexOutput(float3 pos, float2 uv)
        {
            ...
            o.uv=uv;
            o.shadowCoord = TransformWorldToShadowCoord(TransformObjectToWorld(pos));
            return o;
        }
        ...
        TessVertex hull(InputPatch < vertexOutput, 3 > patch, uint id : SV_OutputControlPointID)
        {
            //定义hullshaderV函数
            return patch[id];
        }
        ...
        void geo(triangle vertexOutput IN[3], inout TriangleStream<geometryOutput> triStream)
        {
            ...
            triStream.Append(GenerateGrassVertex(pos, width, 0, 0, float2(0, 0), transformationMatrixFacing));
            triStream.Append(GenerateGrassVertex(pos, -width, 0, 0, float2(1, 0), transformationMatrixFacing));
            float forward = rand(pos.yyz) * _BladeForward;
            for (int i = 1; i < BLADE_SEGMENTS; ++i)
            {
                float t = i / (float)BLADE_SEGMENTS;
                float segmentHeight = t * height;
                float segmentWidth = (1 - t) * width;
            ...
            triStream.Append(GenerateGrassVertex(pos, 0, height, forward, float2(0.5f, 1), transformationMatrix));
